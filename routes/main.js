...
exports.revenuestat=function(req,res){
	...
	var gettrip="select Bill_date,Sum(Total_amount) from Billing where substr(Bill_date,1,10) between '"+sdate+"' and '"+edate+"' group by substr(Bill_date,1,10);";
	mysql.fetchData(function(err, Result){
		if (err) {
			throw err;
		} else{
...
exports.revenuestats=function(req,res)
{
	...
	var gettrip="select Bill_date,Sum(Total_amount) as sum from Billing where substr(Bill_date,1,10) between '"+sdate+"' and '"+edate+"' group by substr(Bill_date,1,10);";
	mysql.fetchData(function(err, Result){
		if (err) {
			throw err;
		} else{
...
exports.upload=function(req,res){
    ...
    var target_path = './uploads/' + req.files.upl.name;
    console.log("aj"+target_path);
	...
	//res.status(204).end();
            ...
            p=target_path;
            console.log("p"+req.session.path);
